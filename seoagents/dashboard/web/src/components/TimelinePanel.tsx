...
const KIND_META: Record<string, { label: string; color: string }> = {
  START: { label: '开工', color: 'var(--accent)' },
  CHECKPOINT: { label: '检查', color: 'var(--rev)' },
  DEADLINE: { label: '截止', color: 'var(--bad)' },
  REVIEW: { label: '复盘', color: 'var(--ok)' },
}
...
const SOURCE_META: Record<string, { label: string; color: string; tip: string }> = {
  ag: { label: 'Ag', color: 'var(--accent2)', tip: 'Agent自主创建' },
  yh: { label: 'Yh', color: 'var(--warn)', tip: '用户手动创建' },
  ya: { label: 'Ya', color: 'var(--rev)', tip: '用户让Agent创建' },
}
...
export const TimelinePanel: React.FC = () => {
  ...
  const loadColor = load_pct > 80 ? 'var(--bad)' : load_pct > 50 ? 'var(--warn)' : 'var(--ok)'

  const nodeRow = (n: Node, actions = true) => {
    const meta = KIND_META[n.kind] || { label: n.kind, color: 'var(--faint)' }
    return (
      <div key={n.node_id} style={{
        display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: 8, background: 'var(--surface)',
        border: '1px solid var(--panel)', borderRadius: 6, padding: '7px 9px', fontSize: 11,
        flexWrap: isMobile ? 'wrap' : 'nowrap',
      }}>
        <span style={{
          flexShrink: 0, background: 'var(--panel)', color: meta.color, borderRadius: 3,
          padding: '1px 6px', fontSize: 9, fontWeight: 700, width: 40, textAlign: 'center',
        }}>{meta.label}</span>
        {(() => { const st = sourceOf(n.created_by); return st ? (
          <span style={{ flexShrink: 0, fontSize: 8, fontWeight: 700, color: st.color, background: 'var(--bg)', borderRadius: 3, padding: '1px 5px', textAlign: 'center' }} title={st.tip}>{st.label}</span>
        ) : null })()}
        <span style={{ flexShrink: 0, color: 'var(--dim)', fontSize: 10, width: 88 }}>{fmt(n.scheduled_at)}</span>
        <span style={{ flex: 1, minWidth: isMobile ? '100%' : 0, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: isMobile ? 'normal' : 'nowrap', order: isMobile ? 3 : 0 }}>
          {n.intent}
        </span>
        <span style={{ flexShrink: 0, color: 'var(--border)', fontSize: 10, width: 40, textAlign: 'right' }}>{n.expected_minutes}分</span>
        <span style={{ flexShrink: 0, color: 'var(--border)', fontSize: 9, minWidth: 62 }}>
          {n.runtime_state || n.state}
        </span>
        {actions && (n.cron?.job_id || n.context?.scheduler === 'hermes-pulse') && (
          <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
            {n.runtime_state === 'BLOCKED_APPROVAL' ? (
              <button onClick={() => act(`/api/timeline/schedules/${n.node_id}/approve`, undefined, '批准执行')} style={miniBtn('var(--warn-soft)', 'var(--warn)')}>批准</button>
            ) : (
              <button onClick={() => act(`/api/timeline/schedules/${n.node_id}/run`, undefined, '立即运行')} style={miniBtn('var(--accent-soft)', 'var(--accent)')}>运行</button>
            )}
            <button onClick={() => act(`/api/timeline/schedules/${n.node_id}/${n.runtime_state === 'PAUSED' ? 'resume' : 'pause'}`, undefined, n.runtime_state === 'PAUSED' ? '恢复' : '暂停')} style={miniBtn('var(--border)', 'var(--text)')}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
        <div style={card}>
          <div style={{ fontSize: 10, color: 'var(--faint)' }}>⏱️ 负载率</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: loadColor }}>{load_pct}%</div>
          <div style={{ background: 'var(--panel)', height: 4, borderRadius: 2, marginTop: 4, overflow: 'hidden' }}>
            <div style={{ width: `${Math.min(load_pct, 100)}%`, background: loadColor, height: '100%' }} />
          </div>
          <div style={{ fontSize: 9, color: 'var(--border)', marginTop: 3 }}>
            已承诺 {agenda?.committed_minutes} 分 / 未来 {agenda?.horizon_hours} 小时
          </div>
        </div>
        <div style={card}>
          <div style={{ fontSize: 10, color: 'var(--faint)' }}>📅 已排节点</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)' }}>{agenda?.upcoming.length ?? 0}</div>
          <div style={{ fontSize: 9, color: 'var(--border)', marginTop: 3 }}>Hermes Cron 执行中 {agenda?.in_flight.length ?? 0}</div>
        </div>
        <div style={card}>
          <div style={{ fontSize: 10, color: 'var(--faint)' }}>⏰ Cron 固定节律</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)' }}>{agenda?.cron_jobs?.length ?? 0}</div>
          <div style={{ fontSize: 9, color: 'var(--border)', marginTop: 3 }}>全部任务最终由 Hermes Cron 执行</div>
        </div>
        <div style={card}>
          <div style={{ fontSize: 10, color: 'var(--faint)' }}>🕳️ 下一个空档</div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11, color: 'var(--faint)' }}>视野</span>
        {[15, 24, 72, 168].map((h) => (
          <button key={h} onClick={() => { setHours(h); load(h) }}
            style={btn(hours === h ? 'var(--accent)' : 'var(--border)')}>{h === 15 ? '15小时' : h === 24 ? '1天' : h === 72 ? '3天' : '7天'}</button>
        ))}
        <div style={{ flex: 1 }} />
        <button onClick={() => setShowNew(!showNew)} style={btn('var(--accent2)')}>＋ 排期</button>
        <button onClick={() => act('/api/timeline/sweep', {}, '清扫')} style={btn('var(--border)')}>🧹 扫过期</button>
        <button onClick={() => act('/api/timeline/triage', {}, '分诊')} style={btn('var(--border)')}>📮 分诊未读</button>
        <button onClick={() => load()} style={btn('var(--border)')}>↻</button>
      </div>

      {msg && <div style={{ ...card, fontSize: 11, color: msg.includes('失败') || msg.includes('拒') ? 'var(--bad)' : 'var(--ok)' }}>{msg}</div>}
      ...
      {showNew && (
        <div style={{ ...card, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={schedule} disabled={!nIntent.trim() || !nAt} style={btn(nIntent.trim() && nAt ? 'var(--accent2)' : 'var(--border)')}>排入</button>

          <div style={{ width: '100%', borderTop: '1px solid var(--panel)', paddingTop: 8, marginTop: 2 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              {([['agent', '新建 Agent 任务'], ['workflow', '执行已有工作流']] as const).map(([m, label]) => (
                <button key={m} onClick={() => setNMode(m)} style={{
                  background: nMode === m ? 'var(--panel)' : 'transparent',
                  color: nMode === m ? 'var(--accent)' : 'var(--dim)',
                  border: `1px solid ${nMode === m ? 'var(--accent)' : 'var(--panel)'}`,
      ...
      {picked && (
        <div onClick={() => setPicked(null)} style={{
          position: 'fixed', inset: 0, background: 'oklch(0% 0 0 / .7)', zIndex: 100001,
          display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16,
        }}>
          <div onClick={(e) => e.stopPropagation()} style={{
            width: 460, maxWidth: '100%', background: 'var(--surface)',
            border: '1px solid var(--panel)', borderRadius: 12, padding: 16,
          }}>
            ...
            {picked.context && Object.keys(picked.context).length > 0 && (
              <pre style={{
                fontSize: 10, color: 'var(--dim)', background: 'var(--bg)', borderRadius: 6,
                padding: 8, marginTop: 8, maxHeight: 160, overflow: 'auto',
              }}>{JSON.stringify(picked.context, null, 2)}</pre>
            )}
            <button onClick={() => setPicked(null)} style={{ ...btn('var(--border)'), marginTop: 10, width: '100%' }}>关闭</button>
          </div>
        </div>
      )}
      ...
      {due.length > 0 && (
        <div style={{ ...card, borderColor: 'var(--warn-soft)' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--warn)', marginBottom: 6 }}>🔔 已到点,等你处理 ({due.length})</div>
      ...
      {(agenda?.in_flight.length ?? 0) > 0 && (
        <div style={card}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent2)', marginBottom: 6 }}>▶ 执行中 ({agenda!.in_flight.length})</div>
      ...
      <div style={card}>
        ...
        {(agenda?.upcoming.length ?? 0) === 0 ? (
          <div style={{ color: 'var(--border)', fontSize: 11, padding: '14px 0', textAlign: 'center' }}>
      ...
      {unread.length > 0 && (
        <div style={card}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--rev)', marginBottom: 6 }}>📮 未读回执 ({unread.length})</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>{unread.map((n) => nodeRow(n, false))}</div>
        </div>
      )}

      <div style={{ fontSize: 10, color: 'var(--border)', textAlign: 'right' }}>
...
const btn = (bg: string): React.CSSProperties => ({
  background: bg, color: 'var(--text)', border: 0, borderRadius: 5, padding: '5px 12px',
...
const input: React.CSSProperties = {
  ...
  color: 'var(--text)', fontSize: 11, padding: '5px 8px', boxSizing: 'border-box',
}
